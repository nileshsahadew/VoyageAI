...
function groupItineraryByDate(itinerary) {
  ...
  for (const date of Object.keys(grouped)) {
    grouped[date].sort(
      (a, b) => parseTimeToMinutes(a.hour) - parseTimeToMinutes(b.hour)
    );
  }
  return grouped;
}
...
function parseToDate(dateStr, timeStr) {
  ...
  return new Date(
    `${dateStr}T${String(hours).padStart(2, "0")}:${String(mins).padStart(
      2,
      "0"
    )}:00`
  );
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.headerBar}>
          ...
          {recipientName ? (
            <Text style={styles.headerSubtitle}>For {recipientName}</Text>
          ) : null}
        </View>
        ...
        {dates.map((date) => (
          <View key={date}>
            <View style={styles.dateHeader}>
              ...
              {grouped[date][0]?.day ? (
                <Text style={styles.dayChip}>{grouped[date][0].day}</Text>
              ) : null}
            </View>
            {grouped[date].map((item, idx) => (
              <View key={`${date}-${idx}`} style={styles.activityCard}>
                {item.hour ? (
                  <Text style={styles.timePill}>{item.hour}</Text>
                ) : null}
        ...
        <Text style={styles.footer}>
          Generated on {new Date().toLocaleDateString()} · AuraDrive Resort
        </Text>
      </Page>
    </Document>
  );
...
export async function POST(req) {
  try {
    ...
    const providedItinerary = Array.isArray(body.itinerary)
      ? body.itinerary
      : [];
    ...
    return NextResponse.json(
      {
        error: "Failed to generate or send itinerary",
